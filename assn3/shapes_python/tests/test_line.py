...
class TestLine(unittest.TestCase):
    ...
    def testValidConstruction(self):
        ...
        p3 = Point(1.4, 2.5)
        p4 = Point(4.6, 10.7)
        l2 = ShapeFactory.build("line", p3, p4)

        self.assertEqual(p3, l2.point1)
        self.assertEqual(p4, l2.point2)

        l3 = ShapeFactory.build("line",Point(1, 3.33), Point(4.444, 5.5555))
    ...
    def testInvalidConstruction(self):
        ...
        with(self.assertRaises(ShapeException)):
            ShapeFactory.build("line", Point(float('inf'), 2), Point(3, 4))
            ShapeFactory.build("line", Point(1, float('inf')), Point(3, 4))
            ShapeFactory.build("line", Point(1, 2), Point(float('inf'), 4))
            ShapeFactory.build("line", Point(1, 2), Point(3, float('inf')))
            ShapeFactory.build("line", p1, p1)
            ShapeFactory.build("line", Point(1, 2), Point(1, 2))
